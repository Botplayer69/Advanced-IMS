import { Package, AlertTriangle, ClipboardList, Truck, ArrowDown, ArrowUp, ArrowDownLeft, ArrowLeftRight, History } from "lucide-react";
          ...
          {kpiData.map((kpi) => (
            ...
            >
              ...
              <div className="flex items-end justify-between">
                ...
                <span
                  className={`flex items-center gap-0.5 text-xs font-mono ${kpi.accent === "destructive"
                      ? "text-destructive"
                      : kpi.accent === "warning"
                        ? "text-warning"
                        : "text-primary"
              ...
              {lowStockItems.map((item) => (
                <div key={item.sku} className="p-4 hover:bg-accent/30 ims-hover">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span
                      className={`text-xs font-mono font-bold ${item.stock === 0 ? "text-destructive" : "text-warning"
                        }`}
                    >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${styles[status] || "bg-muted text-muted-foreground border-border"
        }`}
    >
      {status}
    </span>
